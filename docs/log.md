...
- 2026-07-12: 新增 `concepts/superagent.md` — SuperAgent 长时域智能体架构（源自微信公众号「虾说AI实验室」文章：https://mp.weixin.qq.com/s/zzxzluJ5Jwop2Pp320Hi9w，字节 deer-flow 参考实现）。覆盖：普通 Agent 三大痛点（崩/忘/乱）、六大模块架构（SuperAgent 编排层 / 沙箱运行时 / 三层记忆 / 技能市场 / 工具注册中心 / 消息网关）、关键能力（长时域稳定+断点续传、沙箱隔离、持久记忆、子代理并行、多 LLM 兼容）、工作原理（任务拆解 DAG → 子代理并行 → 消息网关路由 → 三层记忆存储 → 汇总输出）、案例（全球 AI Agent 市场调研并生成 PPT）。核心洞察：**SuperAgent 用"全局调度+沙箱+持久记忆+子代理协作"替代人工拆任务与汇总，专攻小时级长时域任务；其架构与 [[a2a-protocol]] 多代理通信、[[multica-loop-agent]] 全闭环编排、[[openshell]] 沙箱隔离高度呼应**。更新 `index.md`（+1 concept、页数 137→138）。confidence: medium（文章数据如 Star 数为来源单方声称）。
- 关联 [[a2a-protocol]]、[[multica-loop-agent]]、[[agentic-rag]]、[[openshell]]、[[agent-sandbox]]、[[polos]]

- 2026-07-12: 新增 `entities/deer-flow.md` — DeerFlow（字节跳动开源 Super Agent Harness）源码级架构分析。来源：克隆本地仓库 `~/Projects/deer-flow`（2.0 分支，基于 `backend/docs/ARCHITECTURE.md` + `lead_agent/agent.py` + 模块目录实测）。覆盖：三层服务（Nginx 2026 统一入口 → LangGraph Server 2024 / Gateway API 8001 / Frontend 3000）、核心模块（lead_agent 编排 + 8+ Middleware 链 / 子代理 general_purpose+bash_agent 隔离并行 / Sandbox Local+Aio(Docker)+K8s provisioner / Skills 渐进加载 SKILL.md / Tools 三类聚合 / Model Factory 反射 / 长期记忆+ThreadState+Summarization / IM 渠道 Telegram·Slack·Feishu·WeCom / LangSmith+Langfuse Tracing）、技术栈（Python3.12+LangGraph+FastAPI / Next.js / Docker / 默认 127.0.0.1）。核心洞察：**对照微信营销文（虾说AI实验室）做纠偏——真实工程内核是 LangGraph 编排+Middleware 链+沙箱隔离+技能渐进加载，而非其渲染的"自研 DAG 调度 / 知识图谱记忆 / pub-sub 消息总线"，Star 数 74K 亦属未核实营销数字**；与 [[superagent]] 概念主干（编排/沙箱/记忆/子代理）一致但实现层需打折。更新 `index.md`（+1 entity、页数 138→139）；回链 [[superagent]]。confidence: high（基于真实源码）。
- 关联 [[superagent]]、[[a2a-protocol]]、[[multica-loop-agent]]、[[agentic-rag]]、[[openshell]]、[[agent-sandbox]]、[[polos]]

- 2026-07-12: 新增 `concepts/codex-ai-ppt-fangzhenyi.md` + `raw/yitang/codex-ai-ppt-fangzhenyi-原文整理.md` — 方振义(Lester)·用 Codex 做 PPT。来源：一堂飞书 Wiki（Playwright 抓取，21660字）。覆盖：两条 AI 做 PPT 路线对比（HTML 版 PPT=可调试工程 vs Image 版 PPT=设计海报）、HTML 版八步工作流（安装 guizang-ppt-skill→AI 读资料盘点→计划模式生成大纲→查看 index.html 初版→备份与风格变更→截图+注释逐页修改→严格16:9导出PDF/PPT→沉淀项目级 AGENTS.md）、Image 版六步工作流（项目目录→asset_inventory 盘点→Product Design 风格设计 ASCII 线框/风格样板/visual_style_guide→Image Gen 生图→批量生成→逐页修改）、关键原则（非覆盖式生成/每份Word对应1–2页/数据版式只放真实数据/来源注释/项目级而非全局 AGENTS.md）、浏览器验收清单（静态/视觉/交互）。核心洞察：**HTML 像可调试工程、Image 像一张海报，两者解决不同问题；用 AI 编程工具把"做 PPT 初稿"从手工排版变成可拆解可验收可沉淀的流程；最终回到一堂双三角——人类三角(审美/体系/创造力)决定质量上限，AI三角(场景/数据/基本功)决定执行下限**。更新 `index.md`（+1 concept、+1 raw、页数 139→140）。
- 关联 [[yitang-dual-triangle]]、[[kecheng-to-ai-tool]]、[[skill-architect-methodology]]、[[diary-to-book-skill]]
...
## 2026-07-21 | 创建实体页 | entities/openbb.md

- 新增 OpenBB 开源金融数据平台学习笔记，涵盖产品矩阵、openbb-core 架构、安装使用、REST API / MCP Server、Extension 生态、标准化框架、OBBject 输出模型、本地环境状态与下一步建议。
- 关联：[[tanzhen]]、[[truth-verification-competitors]]、[[mcp-protocol]]、[[a2a-protocol]]

## 2026-08-03 · volcano
- **操作**: 创建 Volcano entity 页（深度学习本机 ~/Projects/volcano 源码）
- **文件**: entities/volcano.md
- **内容**: Kubernetes 原生批量调度系统（CNCF 孵化，基于 kube-batch 演进）。核心解决原生 kube-scheduler 缺乏 gang scheduling/队列权重/抢占回收的问题。架构 4 大组件：vc-scheduler（Session 周期调度 + Action 流水线 + Plugin 注入策略）、vc-controller-manager（job/podgroup/queue/jobflow/cronjob/gc/hypernode/sharding/colocation 等 15 控制器）、vc-webhook-manager（Job/PG/Queue CRD 准入）、vc-agent + vc-agent-scheduler + network-qos（节点侧 NUMA/网络QoS 上报）。10 类 CRD：Job/CronJob/JobFlow/JobTemplate（batch）、PodGroup/Queue（scheduling）、Command（bus）、NUMATopology/HyperNode/NodeShard/ColocationConfiguration。核心概念：PodGroup（minMember gang 全或无）、Queue（weight fair-share + hierarchy 多级队列）、Job（minAvailable + tasks + policies + plugins env/ssh/svc + maxRetry）、Session（每 1s 周期隔离快照）、Action+Plugin 正交框架（allocate/backfill/preempt/reclaim/gangpreempt/gangreclaim/shuffle × 28 plugin）、HyperNode/NodeShard/NUMATopology 拓扑亲和、Command 事件总线、ColocationConfiguration 混部。数据流：周期触发→Session.Open 快照→Plugins.OnSessionOpen 预计算→Actions 流水线（enqueue→allocate pipeline 预分配→backfill→preempt/reclaim/gang*→shuffle）→Plugins.OnSessionClose→Session.Close 绑定提交→Controller 反应（PodGroup/Job phase、policies 触发 RestartJob、maxRetry 计数）。持久化全走 etcd+CRD 无独立 DB，调度器状态在 cache 内存。Go 1.26 + Makefile + docker buildx + Helm chart。8 大设计权衡：独立调度器 vs kube-scheduler plugin（gang 决策需跨 Pod 视角）、Session 隔离 vs 长事务（1s 周期吞吐/延迟折中）、Action/Plugin 正交解耦（隐式耦合需 Tier 控制）、CRD 即状态层（etcd 限速靠 sharding 分片）、pipeline 预分配（吞吐换正确性）、混部优先 vs 专享（生产 AI 常关 overcommit）、CGO 插件可选（破坏静态链接默认不开）、生态集成靠 PodGroup 注解不 fork 上游。生态：Spark/Flink/Ray/TF/PyTorch/MPI/MindSpore/PaddlePaddle/Horovod + Argo/Kubeflow 分层组合
- **来源**: [~/Projects/volcano 源码](~/Projects/volcano), [GitHub](https://github.com/volcano-sh/volcano), [官网](https://volcano.sh), [docs/design/execution-flow.md](~/Projects/volcano/docs/design/execution-flow.md)
- **关联**: [[kubernetes-crd]], [[kagent]], [[argo-workflows]], [[kubeflow]], [[apache-airflow]], [[temporal]]

---

## 2026-08-19 · agency-agents
- **操作**: 创建 The Agency entity 页（深度学习本机 ~/Projects/agency-agents 源码）
- **文件**: entities/agency-agents.md
